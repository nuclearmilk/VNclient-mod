...
class _HomePageState extends ConsumerState<HomePage> {
  ...
  void initState() {
    super.initState();
    // Check followed producers for new works shortly after the home page
    // first renders. Fire-and-forget; the result updates the badge.
    WidgetsBinding.instance.addPostFrameCallback((_) {
      Future.microtask(
        () => ref.read(followServiceProvider.notifier).checkForNewWorks(),
      );
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          // Message center bell with a red dot when there are unread items.
          IconButton(
            icon: _NotificationIcon(
              hasUnread: followState.hasUnread,
              unreadCount: followState.unreadCount,
            ),
            tooltip: '消息',
      ...
      body: RefreshIndicator(
        onRefresh: () async {
          ...
          // Re-check followed producers for new works on pull-to-refresh.
...
class _NotificationIcon extends StatelessWidget {
  const _NotificationIcon({required this.hasUnread, required this.unreadCount});
